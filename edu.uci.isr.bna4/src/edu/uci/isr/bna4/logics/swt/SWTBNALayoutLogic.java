...
public class SWTBNALayoutLogic
	...
	private void relayout(IThing thing, Set<String> layedOut){
		...
		if(targetThingID != null){
			updateLayoutContent(targetThingID, thing, thing.getProperty(IHasSWTLayoutData.SWT_LAYOUT_DATA_PROPERTY_NAME), true);
		}
	...
	public void bnaModelChanged(final BNAModelEvent evt){
		...
		case THING_ADDED: {
			...
			if(thing.getProperty(IHasSWTLayoutData.LAYOUT_TARGET_ID_PROPERTY_NAME) != null){
				updateLayoutContent((String)thing.getProperty(IHasSWTLayoutData.LAYOUT_TARGET_ID_PROPERTY_NAME), thing, thing.getProperty(IHasSWTLayoutData.SWT_LAYOUT_DATA_PROPERTY_NAME), false);
			}
