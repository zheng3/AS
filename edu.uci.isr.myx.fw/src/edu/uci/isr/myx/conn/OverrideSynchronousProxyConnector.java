...
public class OverrideSynchronousProxyConnector extends SynchronousProxyConnector{
	...
	public void init(){
		...
		proxyObject = Proxy.newProxyInstance(providedClasses.iterator().next().getClassLoader(), providedClasses.toArray(new Class<?>[providedClasses.size()]), this);
	}
	...
	Set<Class<?>> getInterfaceClasses(IMyxName interfaceName){
		...
		if(miDesc instanceof MyxJavaClassInterfaceDescription){
			...
			classNames.addAll(Arrays.asList(jmiDesc.getServiceObjectInterfaceNames()));
		}
