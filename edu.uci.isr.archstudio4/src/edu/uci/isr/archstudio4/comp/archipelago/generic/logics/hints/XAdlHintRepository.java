...
import edu.uci.isr.bna4.logics.hints.BasicEncodedValue;
import edu.uci.isr.bna4.logics.hints.IHintRepository;
import edu.uci.isr.bna4.logics.hints.IHintRepositoryChangeListener;
import edu.uci.isr.bna4.logics.hints.IPropertyCoder;
import edu.uci.isr.bna4.logics.hints.MasterPropertyCoder;
import edu.uci.isr.bna4.logics.hints.PropertyDecodeException;
import edu.uci.isr.bna4.logics.hints.IPropertyCoder.IEncodedValue;
...
public class XAdlHintRepository
	...
	public XAdlHintRepository(XArchFlatInterface xarch, ObjRef rootElementRef, String maintainer, String version, ThingPropertyTrackingLogic tptl){
		this.xarch = xarch;
		...
		propertyRefTracker.addTrackerListener(new IXArchRelativePathTrackerListener(){
			...
			public void processAdd(ObjRef objRef, ObjRef[] relativeAncestorRefs){
				if(DEBUG){
					System.err.println("Adding   P: " + relativeAncestorRefs[1] + " " + (String)XAdlHintRepository.this.xarch.get(relativeAncestorRefs[0], "name"));
				}
			...
			public void processUpdate(ObjRef objRef, ObjRef[] relativeAncestorRefs, XArchFlatEvent evt, XArchPath relativeSourceTargetPath){
				...
				if(relativeSourceTargetPath.toTagsOnlyString().startsWith("name")){
					if(DEBUG){
						System.err.println("Updating P: " + relativeAncestorRefs[1] + " " + (String)XAdlHintRepository.this.xarch.get(relativeAncestorRefs[0], "name"));
					}
					update(objRef, hintedElementRef);
					if(!ignoreObjRefUpdates.remove(objRef)){
						fireHintRepositoryChangeEvent(hintedElementRef, objRef);
			...
			public void processRemove(ObjRef objRef, ObjRef[] relativeAncestorRefs){
				if(DEBUG){
					System.err.println("Removing P: " + relativeAncestorRefs[1] + " " + (String)XAdlHintRepository.this.xarch.get(relativeAncestorRefs[0], "name"));
				}
				ObjRef hintedElementRef = relativeAncestorRefs[1];
				boolean fireUpdate;
	...
	private void setRootHintedElementRef(boolean create){
		...
		if(!equalz(rootHintedElementRef, hintedElementRefTracker.getRootObjRef())){
	...
	private ObjRef getHintedElementRef(String id, boolean create){
		synchronized(idToHintedElementRefs){
			 ...
			 * event yet that would inform us that the hint is available.
			 */
			hintedElementRefTracker.rescan();
			hintedElementRef = idToHintedElementRefs.get(id);
			if(hintedElementRef != null && xarch.hasAncestor(hintedElementRef, hintedElementRefTracker.getRootObjRef())){
				return hintedElementRef;
			}
			...
			if(create){
				...
				idToHintedElementRefs.put(id, hintedElementRef);
				xarch.add(hintedElementRefTracker.getRootObjRef(), "hintedElement", hintedElementRef);
			}
			return hintedElementRef;
		}
	}
	...
	private ObjRef getPropertyRef(String id, String name, boolean create){
		synchronized(hintedElementRefNameToPropertyRefs){
			...
			if(hintedElementRef != null){
				...
				if(propertyRef != null && propertyRefTracker.getRootObjRef() != null && xarch.hasAncestor(propertyRef, propertyRefTracker.getRootObjRef())){
					return propertyRef;
				}
	...
	ListenerList<IHintRepositoryChangeListener> changeListeners = new ListenerList<IHintRepositoryChangeListener>(IHintRepositoryChangeListener.class);
	...
	protected void fireHintRepositoryChangeEvent(ObjRef hintedElementRef, ObjRef propertyRef){
		...
		if(targetRef != null){
			...
			if(href != null && href.startsWith("#")){
				...
				fireHintRepositoryChangeEvent(context, propertyRef != null ? (String)xarch.get(propertyRef, "name") : null);
			}
		}
	}
}
