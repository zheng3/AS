...
public class BNAComposite
	extends Composite
	implements PaintListener, IBNAModelListener, ICoordinateMapperListener{
		...
		if(hasHscroll()){
			...
			hb.addListener(SWT.Selection, new Listener(){
				...
				public void handleEvent(Event event){
					...
					((DefaultCoordinateMapper)getCoordinateMapper()).repositionAbsolute(sel - getCoordinateMapper().getWorldMinX(), getCoordinateMapper().localYtoWorldY(0));
				}
			});
		}
		...
		if(hasVscroll()){
			...
			vb.addListener(SWT.Selection, new Listener(){
				...
				public void handleEvent(Event event){
					...
					((DefaultCoordinateMapper)getCoordinateMapper()).repositionAbsolute(getCoordinateMapper().localXtoWorldX(0), sel - getCoordinateMapper().getWorldMinY());
				}
			});
		}
	}
	...
	public void paintControl(PaintEvent e){
		...
		try{
			IThing[] things = getWorld().getBNAModel().getAllThings();
			for(int i = 0; i < things.length; i++){
				if(!Boolean.TRUE.equals(things[i].getProperty(IBNAView.HIDE_THING_PROPERTY_NAME))){
					IThingPeer peer = getView().getPeer(things[i]);
