...
import edu.uci.isr.archstudio4.comp.fileman.NewFileWizard;
import edu.uci.isr.archstudio4.comp.resources.IResources;
import edu.uci.isr.archstudio4.editors.XadlEditorMatchingStrategy;
...
public class LauncherView extends ViewPart{
	...
	public void createPartControl(Composite parent){
		...
		resources.createImage(IMAGE_ARCHSTUDIO_LOGO, LauncherView.class.getResourceAsStream("res/archstudio-logo.gif"));
		resources.createImage(IMAGE_NEW_FILE_ICON, Archstudio4Activator.class.getResourceAsStream("res/icon-new-file-32x32.gif"));
		resources.createImage(IMAGE_ARCHSTUDIO_ICON, Archstudio4Activator.class.getResourceAsStream("res/archstudio-icon-32x32.png"));
		resources.createImage(IMAGE_ISR_ICON, Archstudio4Activator.class.getResourceAsStream("res/isr-icon-32x32.gif"));
		...
		bVisitWebPage.setToolTipText("Visit ArchStudio 4 Website");
		...
		/*
		tb.addListener(SWT.MouseExit, new Listener(){
			public void handleEvent(Event event){
				lExplanation.setText(NO_TOOL);
		...
		tb.addListener(SWT.MouseMove, new Listener(){
			public void handleEvent(Event event){
				ToolItem ti = tb.getItem(new Point(event.x, event.y));
				if((ti != null) && (ti.getData() instanceof ILaunchData)){
					ILaunchData ld = (ILaunchData)ti.getData();
					String text = ld.getDescription();
					if(ld.getLaunchType() == ILaunchData.EDITOR){
						text += ".  Drop a file here to begin.";
					}
					else if(ld.getLaunchType() == ILaunchData.VIEW){
						text += ".  Click this button to begin.";
					}
					lExplanation.setText(text);
				}
				else{
					lExplanation.setText(NO_TOOL);
				}
		...
		for(int i = 0; i < tb.getItemCount(); i++){
			...
			if(ti.getData() instanceof ILaunchData){
				final ILaunchData ld = (ILaunchData)ti.getData();
				ti.addListener(SWT.Selection, new Listener(){
					public void handleEvent(Event event){
						if(ld.getLaunchType() == ILaunchData.EDITOR){
							IResource[] res = EclipseUtils.selectResourcesToOpen(getSite().getShell(), 
								SWT.SINGLE, "Choose file to open", new String[]{"xml"});
	...
	protected void createToolBar(Composite parent, Label lDetail){
		...
		GridLayout cBarLayout = new GridLayout(launchData.length, false);
		...
		for(int i = 0; i < launchData.length; i++){
			createToolItem(cBar, launchData[i], lDetail);
		}
	}
	...
	protected void createToolItem(Composite parent, final ILaunchData launchData, final Label lDetail){
		...
		bItem.addListener(SWT.MouseMove, new Listener(){
			public void handleEvent(Event event){
				...
				if(launchData.getLaunchType() == ILaunchData.EDITOR){
					text += ".  Drop a file here to begin.";
				}
				else if(launchData.getLaunchType() == ILaunchData.VIEW){
					text += ".  Click this button to begin.";
				}
