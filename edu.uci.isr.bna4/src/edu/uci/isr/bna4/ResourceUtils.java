...
public class ResourceUtils{
	...
	public static Font getFont(Display display, String name, int size, FontStyle style){
		...
		if(f == null){
			int styleint = 0;
			if(style.equals(FontStyle.NORMAL)) styleint = SWT.NORMAL;
			else if(style.equals(FontStyle.BOLD)) styleint = SWT.BOLD;
			else if(style.equals(FontStyle.ITALIC)) styleint = SWT.ITALIC;
			else styleint = SWT.NORMAL;
	...
	protected static Font getFont(FontRegistry fr, String fontID, FontStyle style){
		if(!fr.hasValueFor(fontID)) return null;
