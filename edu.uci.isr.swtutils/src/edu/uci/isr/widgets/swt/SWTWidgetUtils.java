...
public class SWTWidgetUtils{
	...
	private static String format = "%-75.75s : %,14d\n";
	...
	public static RGB hexToRGB(String hex){
		...
		try{
			return new RGB(Integer.parseInt(hex.substring(0, 2), 16), Integer.parseInt(hex.substring(2, 4), 16), Integer.parseInt(hex.substring(4, 6), 16));
		}
		catch(Throwable t){
			t.printStackTrace();
			return null;
	...
	public static void async(final Display d, final Widget w, final Runnable r){
			...
			public void run(){
				...
				if(!d.isDisposed() && (w == null || !w.isDisposed())){
					...
					if(DEBUG){
						...
						System.err.printf(format, r, time);
					}
				}
			}
		});
		...
		/*
		 * For some reason, the display will occasionally fall asleep and is
		 * never awoken. This makes the system appear as to suddently stop
		 * responding. The fix is to always awake the display thread after
		 * submitting something to run.
		 */
		d.wake();
	}
	...
	public static void sync(Runnable r){
		sync(PlatformUI.getWorkbench().getDisplay(), r);
	}
	...
	public static Object getColumnData(Control c, int columnIndex){
		if(c instanceof Table){
			TableColumn[] columns = ((Table)c).getColumns();
			if(columnIndex < columns.length){
				return columns[columnIndex].getData();
			}
			return null;
		}
		if(c instanceof Tree){
			TreeColumn[] columns = ((Tree)c).getColumns();
			if(columnIndex < columns.length){
				return columns[columnIndex].getData();
			}
			return null;
	...
	public static Object getColumnProperty(ColumnViewer v, int columnIndex){
		Object[] properties = v.getColumnProperties();
	...
	public static void updateColumn(ColumnViewer v, String[] properties){
		...
		if(cp instanceof IStructuredContentProvider){
			...
			v.update(scp.getElements(v.getInput()), properties);
		}
	}

	public static void relayout(Control c){
