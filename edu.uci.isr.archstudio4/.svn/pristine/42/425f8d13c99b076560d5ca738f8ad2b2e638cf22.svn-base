...
public class FeatureEditorSupport{
	...
	public static void setupEditor(ArchipelagoServices AS, ObjRef archStructureRef,ArrayList<String> ids){
		...
		for (int i = 0; i < things.length; i++) {
			//System.out.println("thing ID -->" +things[i].getID());
			Object[] names = things[i].getAllPropertyNames().toArray();
			
			/*for (int j = 0; j < names.length; j++) {
				System.out.println("names --? "+names[j]);
			}*/
			if(things[i] instanceof BoxThing){
			BoxThing thing = (BoxThing)things[i];
			Object on1 = things[i].getProperty("color");
			...
			if(ids.contains(hint.hintContext)){
				things[i].setProperty("color",new RGB(255, 0, 0) );
			}
			
			//thing.setColor(new RGB(255, 0, 0));
			}
		}
