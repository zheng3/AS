...
public abstract class AbstractSWTTreeThingPeer extends AbstractThingPeer{
	...
	protected void updateLocalBoundingBox(ICoordinateMapper cm){
		...
		if(control != null){
			...
			localBoundingBox = new Rectangle(lp.x - (DEFAULT_CONTROL_WIDTH / 2), lp.y - (DEFAULT_CONTROL_HEIGHT / 2), DEFAULT_CONTROL_WIDTH, DEFAULT_CONTROL_HEIGHT);
		}
	...
	public void draw(IBNAView view, GC g){
		...
		if(lt.isEditing() && (control == null)){
			...
			if(composite.isFocusControl()) control.getControl().forceFocus();;
			...
			control.getTree().setSize(350, 400);
