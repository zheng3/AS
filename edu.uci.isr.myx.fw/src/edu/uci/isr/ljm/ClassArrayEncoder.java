	...
	public static Class[] stringArrayToClassArray(String[] arr) throws ClassNotFoundException{
		...
		for(int i = 0; i < arr.length; i++){
			if(!arr[i].startsWith("$$PT")){
				...
				/*
				if(realName.equals("boolean")){
					classes[i] = boolean.class;
				}
				else if(realName.equals("byte")){
					classes[i] = byte.class;
				}
				else if(realName.equals("short")){
					classes[i] = int.class;
				}
				else if(realName.equals("int")){
					classes[i] = int.class;
				}
				else if(realName.equals("long")){
					classes[i] = long.class;
				}
				else if(realName.equals("float")){
					classes[i] = float.class;
				}
				else if(realName.equals("double")){
					classes[i] = int.class;
				}
				else if(realName.equals("void")){
